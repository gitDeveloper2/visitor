...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Mark all apps as eligible for voting since they're in the active launch
    const appsWithVotingStatus = apps.map((app: any) => ({
      ...app,
      inVoting: true,
      launchDate: activeLaunch.date
    }));

    const premium = appsWithVotingStatus.filter((a: any) => a.isPremium);
    const nonPremium = appsWithVotingStatus.filter((a: any) => !a.isPremium);
