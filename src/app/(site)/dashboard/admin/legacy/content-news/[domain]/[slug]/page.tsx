...
export default async function Page({ params }: { params: { domain: string; slug: string } }) {
  ...
  const page = await db.collection('news').findOne({ domain: params.domain, slug: params.slug });
  ...
  return <>
    <Box sx={{ padding: 2 }}>
      ...
      <QuillEditor content={"news"} initialContent={page.content} slug={params.slug} refs={page.refs} faqs={page.faqs} />
