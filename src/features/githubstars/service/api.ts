...
// import {gitstarCache}from "@/features/shared/cache"
import { starSlice } from "@/features/caching/delegator"
import { axiosWithRetry } from "@/features/shared/utils/httputils"
...
// namespace api {
    ...
    export async function getRepoStarRecords(repo: string, token: string, maxRequestAmount: number):Promise<{
        date: string;
        count: number;
    }[]> {
     
        const cachedData = await starSlice.get(repo)as {
            date: string;
            count: number;
        }[];
        if (cachedData) {
        ...
        starSlice.set(repo, starRecords);

        return starRecords
    }

    export async function getRepoLogoUrl(repo: string, token?: string): Promise<string> {
        const cachedData = await starSlice.get(repo+":logo");
        if (cachedData) {
        ...
        starSlice.set(repo+":logo", data.avatar_url);
