// /app/api/tools/[toolId]/suspend/route.ts
...
export async function PATCH(_: Request, { params }: { params: Promise<{ toolId: string }> }) {
  ...
  try {
    await toolService.suspendTool(toolId);
    return NextResponse.json({ message: 'Tool suspended successfully' });
  } catch (error) {
    return NextResponse.json({ error: 'Failed to suspend tool' }, { status: 500 });
  }
}
