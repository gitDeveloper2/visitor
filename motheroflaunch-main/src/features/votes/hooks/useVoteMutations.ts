...
type VoteResponse = { success: true; voted?: true; unvoted?: true };
...
export const useVoteMutation = () => {
  ...
  return useMutation<VoteResponse, Error, string, VoteContext>({
    ...
    onSuccess: (data, _toolId, context) => {
      ...
      queryClient.setQueryData<Record<string, number>>(['votes'], (old) => {
        ...
        return {
          ...old,
          [toolId]: data.voted ? current + 1 : Math.max(0, current - 1),
